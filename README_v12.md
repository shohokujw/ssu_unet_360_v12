...
| 단계 | 결과 |
|---|---|
| step1b | 73 subjects, 41뷰 (seq2 = Gen3_7), ~3초/subject |
| step1c | `max_h/max_l = 0.272` (v11 9뷰의 0.51과 같은 여유 비율) |
| step4 | train 51 / val 11 / test 11, 44 GB |
| step5 | `ndG7_ndD4_dropout_3__fbp_interp`, v11 `model_epoch1300.pth`에서 fine-tune |

### 이 과정에서 고친 v11 버그 두 개

**1. step4가 `params.yml`의 `label_path`를 무시했다.**
프로젝트 안쪽 경로가 하드코딩되어 있어 라벨을 하나도 못 찾고, **경고만 남기고 exit 0**으로
끝났다. 입력만 있고 정답이 없는 데이터셋이 완성된 것처럼 보인다. 이제 설정을 따르고,
라벨이 비면 하드 에러다.

**2. 라벨은 v11 `fin_set`에서 심볼릭 링크로 가져왔다.**
라벨(full-view FBP)은 v11과 v12가 같아야 한다. 재생성하면 미세한 차이가 끼어들 수 있고
44 GB를 더 쓴다. 링크하면 두 실험이 **비트 단위로 동일한 정답**을 쓰는 것이 보장된다.

### 정규화 상수 (train 51 subject)

| | global max | mean p99.9 | 상수 |
|---|---|---|---|
| v11 fbp_lh | 0.465 | 0.127 | 0.51 (= 1.10× max) |
| v12 fbp_interp | 0.235 | 0.072 | **0.272** (= 1.10× max) |

새 분포의 p99.9(0.072)를 그대로 상수로 쓰면 안 된다. v11 상수는 p99.9의 4.01배이므로,
그랬다면 v12 입력만 자기 분포 대비 4배 뜨겁게 들어가 A/B가 정규화 차이를 재게 된다.
