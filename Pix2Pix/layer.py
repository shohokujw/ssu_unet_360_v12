...
class DECNR2d(nn.Module):
    def __init__(self, nch_in, nch_out, kernel_size=4, stride=1, padding=1, output_padding=0, norm='bnorm', relu=0.0, drop=[], bias=[]):
        ...
        # TODO
        layers += [nn.ConvTranspose2d(nch_in, nch_out, kernel_size=kernel_size, stride=stride, padding=padding, output_padding=output_padding, bias=bias)]
        if norm != []:
            layers += [Norm2d(nch_out, norm)] 
...
class ReLU(nn.Module):
    def __init__(self, relu):
        ...
        if relu > 0:
            self.relu = nn.LeakyReLU(relu,inplace=False)
        elif relu == 0:
